...
export function smsToTransaction(parsed, msg, accountLabel = 'من الرسائل') {
  ...
  t.hash = hashTx({ ...t, ref: smsRef(msg, t.id) });
  return t;
}

/**
 * هويّةُ الرسالة مضمونُها، لا لحظةُ وصولها.
 *
 * كانت البصمة تُبنى من معرّف الإيداع، والصندوق يعطي كلَّ إيداعٍ معرّفًا
 * جديدًا — فالرسالة الواحدة إن وصلت مرتين حُسبت مرتين: بإعادة محاولةٍ من
 * أتمتة الجوال، أو بأتمتتين يلتقط شرطاهما رسالةً واحدة. ومسارُ اللصق كان
 * يشتقّ من المضمون فينجو، فيفترق المساران في الرسالة الواحدة فتُضاعَف.
 *
 * فصارا على هوية واحدة: نصُّ الرسالة مطويًّا ومضغوطَ الفراغ. ورسالتان
 * لعمليتين مختلفتين لا تتطابقان — كلُّ قوالب المصارف الثلاثة تحمل وقتها.
 */
function smsRef(msg, fallback) {
  const body = foldArabic(String(msg?.text || '')).replace(/\s+/g, ' ').trim();
  return body ? `sms:${body}` : `sms:${msg?.id || fallback}`;
}
