 ...
 * رقم العملية — إن وُجد — يميّز العمليات المتطابقة في اليوم والمبلغ والوصف،
 * وإلا فيُميّزها ترتيبُ ظهورها (occ) داخل الملف نفسه.
 */
export function hashTx(t, occ = 0) {
  const key = t.ref
    ? `${t.account || ''}|${t.ref}`
    : `${t.date}|${Math.round((t.amount || 0) * 100)}|${(t.desc || '').slice(0, 60)}|${t.account || ''}|#${occ}`;
  let h = 5381;
